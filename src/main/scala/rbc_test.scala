...
object rbc_test {
  ...
  def loadProductDF() = {
    spark.read
      ...
      .csv("src/main/resources/data/rbcfiles/product.csv")
  }
  ...
  def loadLocationDF() = {
    ...
    .option("header", "true")
    .option("inferSchema", "true")
    .csv("src/main/resources/data/rbcfiles/location.csv")
  }
  ...
  def loadTransactionDF() =  {
    spark.read
      .option("header", "true")
      .option("inferSchema", "true")
      .csv("src/main/resources/data/rbcfiles/trans*.csv")
  }
   ...
   */
  def totalSalesDF() = {
   loadTransactionDF.selectExpr("sum(sales * units) as TotalSales").show()
  }
   ...
   */
  def topNStoresWithHighestSales(topN: Int) = {
    ...
    finalDF.withColumn("TotalSales", format_number(col("TotalSales"), 2)).show(topN)
   ...
   * @param topN number for the top entries
   */
  def topNCategoriesWithHighestSales(topN: Int) = {
  ...
  def topPerformingStoresByProvince(topN: Int) = {
    ...
    salesByProvStoreDF.withColumn("TotalSales", format_number(col("TotalSales"), 2))
      .withColumn("avgProvSales", format_number(col("avgProvSales"), 2)).show()
  }
  ...
  def main(args: Array[String]) : Unit = {
    totalSalesRDD()
    totalSalesDF()
    topNStoresWithHighestSales(20)
    topNCategoriesWithHighestSales(20)
    topPerformingStoresByProvince(5)
    topPerformingCategoriesByDept(5)
  }
